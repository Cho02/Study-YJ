...
function formatWord(page) {
  ...
  // ★ SRS FSRS 3중 상태 (rich_text JSON)
  const srsRaw = propText(p['SRS']);
  let srs = null;
  if (srsRaw) {
    try { srs = JSON.parse(srsRaw); } catch (_) { /* 유효하지 않은 JSON은 무시 */ }
  }
  return {
    id: page.id,
    ...
    onyomi: onyomi === '' ? null : onyomi, // 음독 (예: "ベンキョウ")
    kunyomi: kunyomi === '' ? null : kunyomi, // 훈독 (예: "まなぶ")
    srsLevel: srsLevel === '' ? 1 : Number(srsLevel) || 1,
    reviewCount: reviewCount === '' ? 0 : Number(reviewCount) || 0,
    srs: srs, // FSRS 3중 상태 JSON (null이면 없음)
  };
}
...
async function createWord(word) {
  ...
  if (word.onyomi) properties['음독'] = { rich_text: [{ text: { content: String(word.onyomi) } }] };
  if (word.kunyomi) properties['훈독'] = { rich_text: [{ text: { content: String(word.kunyomi) } }] };
  ...
  // ★ SRS FSRS 3중 상태 저장
  if (word.srs != null) {
    const srsStr = typeof word.srs === 'string' ? word.srs : JSON.stringify(word.srs);
    properties['SRS'] = { rich_text: [{ text: { content: srsStr.slice(0, 2000) } }] };
  }
  return createPage(properties, {
...
async function updateWord(pageId, word) {
  ...
  if (word.example != null) properties['예문'] = { rich_text: [{ text: { content: String(word.example).slice(0, 2000) } }] };
  if (word.level) properties['레벨'] = { select: { name: String(word.level) } };
  if (word.onyomi != null) properties['음독'] = { rich_text: [{ text: { content: String(word.onyomi).slice(0, 500) } }] };
  if (word.kunyomi != null) properties['훈독'] = { rich_text: [{ text: { content: String(word.kunyomi).slice(0, 500) } }] };
  if (word.srsLevel != null) properties['SRS 레벨'] = { number: Number(word.srsLevel) || 1 };
  if (word.reviewCount != null) properties['회독'] = { number: Number(word.reviewCount) || 0 };
  // ★ SRS FSRS 3중 상태 갱신
  if (word.srs != null) {
